...
- [x] (2026-03-13 00:20Z) Implement the LLVM JSON parser, Git diff reader, region-to-diff intersection logic, console reporting, Markdown summary reporting, and threshold evaluation for changed-region coverage, including tighter report details and broader parser fixture variety for the v1 Rust path.
- [x] (2026-03-13 00:28Z) Add unit tests, fixture tests, and copied-fixture CLI integration tests using temporary working directories and immutable checked-in fixtures for the implemented v1 Rust scenarios, with live-repo manipulation in temp directories and fixture-backed end-to-end CLI coverage for supported switches.
- [x] (2026-03-13 00:40Z) Capture final validation evidence, extract canonical testing process guidance to `docs/TESTING.md`, link that guidance from `AGENTS.md`, and move this plan to `docs/exec-plans/completed/` for closeout.
...
- Decision: The first implementation will gate only changed-region coverage from LLVM JSON, even though the architecture must anticipate line, branch, and future metric types.
...
- Decision: The architecture will separate coverage parsing, diff parsing, metric computation, gating, and output rendering into distinct modules.
  Rationale: Future support for other metrics and report formats should be additive. A monolithic implementation would make later support for line, branch, Cobertura, or other native JSON formats much harder to add safely.
  Date/Author: 2026-03-07 / Codex
...
Define the internal model first. The model must not hard-code region coverage as the only possible metric even though v1 only computes changed-region coverage. Avoid names, enums, structs, helper functions, and renderer contracts that imply the active metric is always region-based. It should have a way to distinguish metric families such as region, line, and branch coverage, and it should represent thresholds independently from CLI parsing so later expansion remains additive. It should also define a parser-neutral source span representation that can at least hold a repository-relative file path plus start and end line information, because diff intersection and report rendering both need that information even when future formats provide richer column-level data.
...
The metric layer should return both the aggregate ratio and the underlying uncovered changed opportunities so the renderers can explain failures. Even though line and branch gates are out of scope for implementation, the metric layer must already be structured so future metric calculators can be added without changing the diff or parser modules.
...
1. Define the internal types and module boundaries before implementing parsers.
    ...
    Expected outcome: Unit tests prove that source spans, metric families, thresholds, and gate-facing result data can represent future line, branch, and region metrics even though only region metrics are actually computed in v1. Tests should explicitly guard against region-specific assumptions leaking into shared types.
...
- additional metric kinds such as line and branch
...
Revision note: Extracted the reusable testing process from this ExecPlan into `docs/TESTING.md`, linked it from `AGENTS.md`, and closed out this plan by marking the remaining milestones complete and moving the file to `docs/exec-plans/completed/`.
