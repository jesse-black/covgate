...
Save this in-progress ExecPlan at `docs/exec-plans/active/covgate-vitest-istanbul-native-json-fixture.md` while the work is being designed or implemented in this repository.
...
- [ ] Define fixture layout under `tests/fixtures/vitest/` with at least one pass and one fail scenario (`repo/`, `overlay/`, checked-in Istanbul JSON artifact).
- [ ] Implement/extend xtask fixture regeneration for `vitest/*` scenarios using native `vitest run --coverage` with the default v8 coverage provider and deterministic artifact normalization.
...
1. `covgate` parses Istanbul-native JSON generated by Vitest v8 coverage and computes changed-line and changed-branch metrics where data exists.
2. `tests/fixtures/vitest/` includes at least one pass and one fail scenario with checked-in native JSON artifacts and clear regeneration instructions.
3. CLI integration tests validate Istanbul behavior in the same shared fixture matrix used by LLVM and Coverlet fixtures.
4. Coverage JSON autodetect accepts LLVM, Coverlet, and Istanbul payloads, and unknown payloads return actionable errors.
5. Repository validation remains green via `cargo xtask validate`.
