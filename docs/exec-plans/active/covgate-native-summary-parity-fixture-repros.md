...
- [~] (2026-03-19 18:23Z) Added regression coverage that asserts native summary artifacts exist for the repro fixtures; still need a stronger mismatch-specific assertion if a checked-in real-tool fixture demonstrates helper divergence on this toolchain.
...
Implementation is partially complete. The repository now has dedicated `.NET` and Vitest repro fixtures with checked-in native summary artifacts, xtask regeneration writes those artifacts deterministically, and line-summary parity tests prefer the captured native summaries whenever they exist.

The remaining gap is stronger proof that the old helper oracle diverges on an actual checked-in tool-generated repro for every targeted ecosystem. On this toolchain, the new Vitest repro fixture still produced the same aggregate line totals in both the helper and native summary output, so the oracle replacement is complete but the helper-divergence demonstration remains weaker than originally planned.
