...
- [ ] Evaluator observation (non-blocking): CLI integration test helpers (`run_covgate_with_env`, `run_covgate_raw`, `run_covgate_raw_with_path`) inherit the full parent environment. Best practice for CLI integration tests is `env_clear()` plus an explicit allowlist of required vars (at minimum `PATH`), which makes tests hermetic and prevents future ambient-env pollution bugs of this class. The targeted `env_remove("GITHUB_STEP_SUMMARY")` fix is pragmatic but reactive — any future feature that reads a new env var will silently inherit it until another bug surfaces. Worth a dedicated cleanup if env-related test failures recur.
