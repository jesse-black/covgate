...
- Plan 25's covgate test runner builder now uses `Command::env_clear()`, restores only `PATH` and `LLVM_PROFILE_FILE`, and applies explicit `.env(...)` overrides. That addresses the plan 27 post-merge `GITHUB_STEP_SUMMARY` inheritance finding more completely than the originally suggested `env_remove("GITHUB_STEP_SUMMARY")` helper edits.
- Added `github_step_summary_matching_explicit_output_writes_once` as the TDD regression for the same-file explicit markdown + `GITHUB_STEP_SUMMARY` case; it failed before the production fix with two `## Covgate` headings and passes after deduplicating same destination paths.
- Validation after addressing the second open finding passed on 2026-05-11: `cargo test --test cli_interface github_step_summary_matching_explicit_output_writes_once`, `cargo test --test cli_interface github_step_summary`, `cargo test --test cli_interface markdown_output`, `cargo test --lib comparable_path`, `cargo test --lib same_destination_detects_existing_file_paths`, and `cargo xtask validate`.
